# Evolution-Native Agent Architecture — v0.3.4-candidate

Status: **IMPLEMENTATION CANDIDATE / NOT CURRENT / NOT MAINLINE / NOT PROMOTED**

This directory is a self-contained candidate successor to ENA v0.3.3. It exists to test a field finding from v0.3.3 adoption: ENA must compile into persistent operating behavior instead of remaining a per-task document-retrieval ritual.
...
`ENA candidate: v0.3.4-candidate`
...
6. Do **not** persist the whole release into always-loaded context merely to claim adoption.
7. If no real task exists yet, do not invent a production scenario. Complete only the adoption work that is real, report the persistence limitation truthfully, and wait for a real task.
...
4. Retrieve the canonical candidate text when reality changed, the boundary is novel/ambiguous, local understanding may be stale, or exact contract/schema/tool semantics are decision-critical.
