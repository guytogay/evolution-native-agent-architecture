# ENA v0.3.7 candidate.0 — 请先阅读

状态：**WORKING_CANDIDATE / NOT_CURRENT / NOT_FROZEN / NOT_RELEASED**

当前唯一面向采用者的 Current 仍然是 `releases/current/` 下的 v0.3.6。

本目录是 v0.3.7 candidate.0 的简体中文语义/运行投影，用来测试：在不增加无必要 Core 条文的情况下，ENA 能否通过具体 HOW 路由变得更可住、更可执行。
...
candidate.0 打包了 Retrieval Obligation、WAIT、Authority Lease、Effect Lifecycle、Recovery Adapter、Evidence Envelope、Evidence Dependency Map、Contested Authorship 等 reference。
...
candidate.0 仍需 identity reconciliation、author adversarial、exact pre-freeze validation、freeze、fresh independent semantic falsification、reconciliation 和 explicit release decision。
