...
Birth base / predecessor frozen source: `bda470e0a6b170cec61225a905957a501454a2fe`.

Round-1 gate `33149597432` / cargo `55e08740fa2e4b033cfb5bd9e8f7a4214a479f08`:

- preserve legacy minimal composed Authority grants while honoring explicitly represented richer revocation/time/scope semantics;
- prevent later non-terminal Effect receipts from silently erasing known COMMITTED/COMPENSATED occurrence truth;
- apply local-equivalent integration chronology/snapshot consistency to transferred source history without upgrading imported evidence to receiver-local proof;
- make the inherited regression harness/result truthfully name candidate-local implementation bytes;
- retain inherited composed-validator compatibility controls at 164/164 plus successor closure 61/61.

Round-2 package reconciliation updates candidate identity, lineage, changelog, release narration, and zh-CN projection status without broadening executable semantic scope.

Current remains v0.3.6. Candidate.3 is not frozen or released.

---

## v0.3.7 candidate.2 — FROZEN / NEEDS_REVISION / NOT_CURRENT / NOT_RELEASED

Candidate.2 succeeded frozen candidate.1, passed exact pre-freeze validation, and was externally frozen at source `bda470e0a6b170cec61225a905957a501454a2fe` / subtree `d5fefc8c786d7e40b3e9a59211ee7045bccee5bf`.

Fresh candidate.2 independent review later sealed:
...
Candidate.2 remains immutable occurrence truth. Its defects are repaired only in successor candidate.3.
