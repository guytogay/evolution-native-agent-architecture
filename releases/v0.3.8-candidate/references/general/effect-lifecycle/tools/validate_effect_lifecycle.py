...
def validate_record(record: dict) -> list[str]:
    ...
    # Terminal occurrence knowledge is monotonic absent an explicit correction
    # representation. A later non-terminal receipt cannot silently erase an
    # already represented COMMITTED/COMPENSATED occurrence and reopen retry.
    for effect_id in effect_by_id:
        ordered_receipts = sorted(
            [r for r in receipt_by_id.values()
             if r.get("effect_id") == effect_id and isinstance(r.get("sequence"), int)],
            key=lambda r: r["sequence"],
        )
        terminal_sequence: int | None = None
        for receipt in ordered_receipts:
            status = receipt.get("observed_status")
            if status in {"COMMITTED", "COMPENSATED"}:
                terminal_sequence = receipt["sequence"] if terminal_sequence is None else min(
                    terminal_sequence, receipt["sequence"]
                )
            elif terminal_sequence is not None and receipt["sequence"] > terminal_sequence:
                errors.append(
                    f"{receipt.get('receipt_id')}: non-terminal receipt {status} occurs after known terminal receipt for {effect_id}"
                )
