# ENA 运行时采用核心 — v0.3.8-candidate.0

状态：`CANDIDATE / NOT_CURRENT / NOT_FROZEN / HOT_SEMANTIC_CUE_SURFACE`。

已发布 Current 仍是 `v0.3.7 / CURRENT / FIELD_VALIDATION`。本文件属于 v0.3.8 successor candidate，不代表已经发布。
...
这些是检索/注意力 cue，不是口号代替 HOW，也不代表全部都有硬代码控制。
...
-> operational/REFERENCE-INDEX.yaml（需要精确实现/引用路径时）
...
- 多组件/多 Agent 组合后结果可能变化 -> composition/emergence 检查；
- 新 Host/model/language -> 重新检查 Host mapping / applicability。
...
正面/负面选择来自 represented reality contact，**默认只属于实际环境**；来源选择、流行程度或其他 Host 的成功不能自动变成当前环境结论。
...
## 9. Composition / Continuity / Standing / Authorship

组件各自有效或改进，不代表组合后的系统一定改进；interaction 会改变决策时，应观察 composed subject。
...
restore 后有后果地 resume 之前，要按需确认 external effect settlement、相关历史是否足够以及 current authority。

`cancel != rollback != compensation`

能救回来不等于有权审批以后所有 variation；当自我变异可能破坏自身 recovery 且后果值得时，应让 rescue path 保持在 damaged variation 之外可达，并把 rescue authority 保持窄。

能力、身份、记忆、reputation、restore 或 credential possession 都不会自动生成当前外部 authority。

## 11. Governance closure / Control retirement

治理底层边界，不管理所有生态互动。

当 control 可能已经过时，不用年龄/事故次数魔法；使用 `operational/procedures/CONTROL-RETIREMENT.md` 恢复原 failure、检查 replacement/coverage，**用 evidence + lineage 决定退役**，并保留 reactivation condition。

当新的 bounded check 已不可能改变决策，就停止叠加治理。

## 12. Candidate 工具边界

v0.3.8 candidate.0 继承 v0.3.7 的主要实用 v2 路径：
...
局部 selection 来自 reality contact，默认只属于实际环境；不从流行、导入或来源结论制造 receiver-local proof。
能力、身份、记忆、reputation、restore 或 credential possession 都不会自动生成当前外部 authority。
外部 effect 未决时先 QUERY/WAIT，不盲 replay；cancel、rollback、compensation 不是同一件事。
当自我变异可能破坏 recovery 且后果值得时，让 rescue path 保持在 damaged variation 之外可达，并保持 rescue authority 狭窄。
退役旧 control 要靠 evidence + lineage，不靠年龄、安静或事故次数魔法。
治理必须闭环并支付成本。
```
