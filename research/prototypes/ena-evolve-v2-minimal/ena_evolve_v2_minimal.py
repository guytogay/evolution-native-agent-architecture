#!/usr/bin/env python3
"""Research-only minimal v2 evolution helper.

This prototype deliberately does not recreate the legacy ena_evolve.py state
machine. It orchestrates existing v0.3.6 v2 record semantics and adds only the
small packet-v2 mechanics that Current does not expose as a standalone tool.

Boundaries:
- evolution-record semantic consistency is delegated to Current's
...
TEMPLATE_PATH = CURRENT / "templates" / "evolution-record.v2.json"
PACKET_SCHEMA_PATH = CURRENT / "schemas" / "adaptation-packet.v2.schema.json"
RECORD_VALIDATOR_PATH = CURRENT / "tools" / "validate_evolution_record_v2.py"
...
def _load_current_record_validator():
    spec = importlib.util.spec_from_file_location("ena_current_record_v2_validator", RECORD_VALIDATOR_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("Unable to load Current evolution-record v2 validator")
...
def validate_record_v2(record: dict[str, Any]) -> list[str]:
    return list(_load_current_record_validator()(record))
...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="ena_evolve_v2_minimal.py", description="Research-only minimal ENA v0.3.6 v2 record/packet helper")
