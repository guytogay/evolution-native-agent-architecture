...
## Assigned update policy: C2 — CONTEXT_REVERSIBLE3
...
- Once a source is incumbent, reverse trust only after 3 consecutive wins by the opposite source in that context.
- One or two contradictory wins are provisional noise and do not immediately rewrite the incumbent preference.
- For an unseen context with no incumbent, use `UNRESOLVED`; if the sources conflict there, answer `INSUFFICIENT`.
